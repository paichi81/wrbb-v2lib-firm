...
int fileloader(const char* str0, const char* str1)
{
	...
	while(true){
		...
		if (CommandData[0] == 'Z' || CommandData[0] == 'z'){
			EEP.format();
		}
		...
		else if(CommandData[0] == 'X' || CommandData[0] == 'V' || CommandData[0] == 'x'){
			if(strlen(CommandData) > 3){
				//スペースを0に変えて、ポインタを取得
				int j = 0;
				int len = strlen(CommandData);
		...
		else if (CommandData[0] == 'L' || CommandData[0] == 'l'){
			USB_Serial->println();
			for(int i=0; i<64; i++){
		...
		//<------- koko made
		*/
		else if (CommandData[0] == 'C' || CommandData[0] == 'c'){
			USB_Serial->println();
		...
		else if (CommandData[0] == 'T' || CommandData[0] == 't'){
			...
			//			USB_Serial->println(" i:I2C DETECT.............>i [ENTER]");
